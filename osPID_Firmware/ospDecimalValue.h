...
/*
 * This class encapsulates the formatting and range information about all the
 * decimal values the Controller uses, and is designed to be stored in PROGMEM.
 */
struct ospDecimalValueInfo {
  char pmemIcon;
  byte pmemFlags;
  void *pmemValPtr;
...
// the list of items in the decimalValueInfo[] array
// NOTE: the first N items of this MUST match the decimal chunck of ITEM_*
// in osPID_Menu.ino!
enum {
  VALUE_SETPOINT = 0,
  VALUE_INPUT,
  ...
  VALUE_KI,
  VALUE_KD,
  VALUE_LOWER_TRIP_LIMIT,
  VALUE_UPPER_TRIP_LIMIT,

  // values with no matching ITEM_*
  VALUE_AUTOTUNER_STEP_SIZE,
  VALUE_AUTOTUNER_NOISE_AMPLITUDE,

  NR_VALUE_INFO_ITEMS
};

extern ospDecimalValueInfo decimalValueInfo[];

#endif // #ifdef HOSTED_TEST
